...
class IDOA:
    ...
    def calc_idoa_vector_custom(self, second_cohort_ind_dict):
        """
        param second_cohort_ind_dict: Dictionary of indexes of the second cohort as a keys and indexes of
        the first cohort as a values, if i --> j so the IDOA is not calculated between this two samples in the process.
        :return: IDOA vector.
        """
        for i in range(0, self.num_samples_second):
            o_vector = []
